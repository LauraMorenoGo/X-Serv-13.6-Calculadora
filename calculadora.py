...
if len(sys.argv) != 4:
    ...
    for v in range(len(operaciones)):           #Comprueba que el primer argumento esté en la lista de las operaciones que se pueden hacer
    ...
    try:      
        if sys.argv[1] == 'sumar':                  #Suma
            ...
            try:
                resultado = float(sys.argv[2]) / float(sys.argv[3])
                print(resultado)
            except ZeroDivisionError:               #En la división nos puede saltar la excepción si el denominador es 0
                print('Introduzca como segundo operando un número distinto de cero')
    except ValueError:
        print('El argumento 2 y 3 tienen que ser números')  #Salta excepción si los argumentos 2 y 3 no son números
